...
from dqnselector.baselines import degree_greedy, celf
...
def parse_args():
    ...
    p.add_argument('--model'); p.add_argument('--budgets',type=int,nargs='+',default=[5,10,20,30,50])
    p.add_argument('--selection-mc',type=int,default=10); p.add_argument('--evaluation-mc',type=int,default=30); p.add_argument('--seed',type=int,default=2026)
    p.add_argument('--hidden',type=int,default=64); p.add_argument('--atoms',type=int,default=31); p.add_argument('--device',default='cuda'); p.add_argument('--skip-celf',action='store_true')
    return p.parse_args()


def main():
    a=parse_args(); out=Path(a.output); out.mkdir(parents=True,exist_ok=True)
    inst,_=load_journal_instance(a.instance); pool=sorted(inst.worker_pool); maxk=min(max(a.budgets),len(pool))
    ...
    for name,order in methods.items():
        for k in sorted(set(a.budgets)):
            ...
            print(name,'k',k,'mean',f"{r['mean']:.6f}",'p10',f"{r['p10']:.6f}",'unsat',f"{r['unsatisfied_ratio']:.3f}",'sat',f"{r['saturated_ratio']:.3f}",'sel',f"{selection_seconds[name]:.3f}")
    with (out/'comparison.csv').open('w',newline='') as f:
        w=csv.DictWriter(f,fieldnames=rows[0].keys()); w.writeheader(); w.writerows(rows)
    summary={'selection_mc':a.selection_mc,'evaluation_mc':a.evaluation_mc,'selection_seconds':selection_seconds,'graph_nodes':inst.graph.number_of_nodes(),'graph_edges':inst.graph.number_of_edges(),'worker_pool':len(pool)}
