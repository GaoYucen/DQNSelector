...
def parse_args():
    p = argparse.ArgumentParser(description="Train size-aware DQNSelector scientific-v5")
...
def main():
    ...
    result = {
        "profile": profile,
        "training_mode": "size_aware_balanced_multibudget",
        ...
        "note": (
            "Scientific-v5 keeps the scientific-v2 objective/instances and balanced multi-budget "
            "training, but represents the selected set by mean-pooled gated embeddings plus an "
            "explicit normalized cardinality. This removes depth-dependent state-scale drift."
        ),
    }
