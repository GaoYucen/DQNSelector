...
def parse_args():
    ...
    p.add_argument('--v-min',type=float,default=0.0); p.add_argument('--v-max',type=float,default=1.0)
...
def main():
    ...
    model=RainbowSelector(social,task,worker_pool=inst.worker_pool,hidden_dim=a.hidden,atoms=a.atoms,v_min=a.v_min,v_max=a.v_max)
    ...
    metrics={'device':device,'episodes':a.episodes,'train_budget':a.train_budget,'oracle_mc':a.oracle_mc,'eval_mc':a.eval_mc,'task_embedding':a.task_embedding,'embedding_mc':a.embedding_mc,'embedding_seconds':embedding_sec,'oracle_build_seconds':oracle_build,'training_seconds':train_sec,'atoms':a.atoms,'v_min':a.v_min,'v_max':a.v_max,'episode_returns':stats.episode_returns,'losses':stats.losses,'evaluation':evaluation,'selection_order':order,'instance_config':man.get('config',{}),'variant':man.get('variant',{})}
